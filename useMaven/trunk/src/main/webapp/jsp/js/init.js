	...
	$.ajax({	url : "getUserMenus.action",
				...
				success : function(_menus) {
					...
					$.each(_menus,function(i, n) {
						if(n.parentId=="0"){
							menulist += '<div title="'
								+ n.name+'" data-options="iconCls:\''+ n.icon
							...
							$.each(_menus,function(index,item){
								if(item.parentId==n.id){
									menulist += '<li><div><a href="###" rel="'
										+ item.url+ '" ref="'
										+ item.id+ '"><span class="'
										+ item.icon	+ '" style="width: 16px; display: inline-block">&nbsp;</span><span class="nav">'
										+ item.name	+ '</span></a></div></li>';									
								}								
							});			
							menulist += '</ul></div>';
						}
